...
public class AllMemberServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		out.println("<!DOCTPYE html>");
		out.println("<html><head><title>전체회원조회</title><head>");
		out.println("<body>");
		if (list.isEmpty()) {
			out.println("<h2>회원정보를 조회 할 수 없습니다.");
		} else {
			...
			for(Member m : list) {
				out.print("<tr>");
				out.print("<td>"+m.getMemberNo()+"</td>");
				out.print("<td>"+m.getMemberId()+"</td>");
				out.print("<td>"+m.getMemberPw()+"</td>");
				out.print("<td>"+m.getMemberName()+"</td>");
				out.print("<td>"+m.getMemberPhone()+"</td>");
				out.print("<td>"+m.getMemberAddr()+"</td>");
				out.print("<td>"+m.getMemberLevel()+"</td>");
				out.print("<td>"+m.getEnrollDate()+"</td>");
				out.print("</tr>");
			}
			out.println("</table>");
		}
