...
public class MailSender {
	...
	public boolean sendMail1(String mailTitle, String receiver, String mailContent) {
		...
		Session session = Session.getDefaultInstance(prop, new Authenticator() {
			protected PasswordAuthentication getPasswordAuthentication() {
				PasswordAuthentication pa = new PasswordAuthentication("testtestplz306", "yadevhbcnalatsih");
				return pa;
			}
		});
		// 이메일을 작성해서 전송하는 객체
		...
		try {
			...
			// 보내는사람 정보
			msg.setFrom(new InternetAddress("testtestplz306@gmail.com", "KH 당산 A클래스"));
			// 받는사람정보
	...
	public String sendMail2(String email) {
		...
		try {
			...
			msg.setRecipient(Message.RecipientType.TO, to);
			// 이메일 제목설정
			msg.setSubject("JW's Page 인증메일입니다.", "UTF-8");
			// 이메일 본문설정
			msg.setContent("<h1>안녕하세요. JW's Page 입니다.<h1>" + "<h3>인증번호는 [<span style = 'color:red'>" + sb.toString()
